...
class Administrator {
    ...
    async processMsyncMessage(socket,content){
        ...
        const PISNtoComp = parts[1];
         ...
         if (T <= 10000) {
             ...
             const resultForPI = await collection23.find({ clientID: this.SNID }).toArray();
             const newPIDs = resultForPI.map(entry => entry.pseudoIdentity);
             const oldPIDs = resultForPI.map(entry => entry.psuedoIDold);
             const newPI = newPIDs[0]; const oldPI = oldPIDs[0];
            ...
            //Synchronisation Check
             ...
             const resultForPI1 = await collection23.find({ clientID: this.SNID }).toArray();
             const newPIDs1 = resultForPI1.map(entry => entry.pseudoIdentity);
             const oldPIDs1 = resultForPI1.map(entry => entry.psuedoIDold);
             const newPI1 = newPIDs1[0]; const oldPI1 = oldPIDs1[0];
             ...
             //Performing XOR between (gamma,neta) to avoid traceability attack- 

            //  const secretBytes1 = [];
            //  const keyForXOR = this.keyForSN
             
            //  for (let i = 0; i < keyForXOR.length; i += 2) {
            //      const byte = parseInt(keyForXOR.substr(i, 2), 16);
    ...
    async processM2GSMessage(socket, content) {
        ...
        if (T <= 10000) {
            ...
            let respnot = op1.map((num, index) => num ^ secretBytes[index]);
    ...
    async processM3UGMessage(socket, content) {
        ...
        if (T <= 10000) {
            ...
            let midhash = this.xorStrings(delta, this.gatewaySecret);
    ...
    async processM1UGMessage(socket, content) {
        ...
        if (T <= 10000) {
            ...
            try {
                ...
                const resultForPI1 = await collection13.find({ clientID : cID}).toArray();
                const newPIDs1 = resultForPI1.map(entry => entry.pseudoIdentity);
                const oldPIDs1 = resultForPI1.map(entry => entry.psuedoIDold);
                const newPI1 = newPIDs1[0]; const oldPI1 = oldPIDs1[0];
                ...
                const resultForKeyold = await collection43.find({ pseudoIdentity: pI }).toArray();
                ...
                // //Peforming XOR of (beta,j) with key to mitigate traceability attack
                // const secretBytes1 = [];
                // const keyforXOR = this.keyForAES;
                // for (let i = 0; i < keyforXOR.length; i += 2) {
                //     const byte = parseInt(keyforXOR.substr(i, 2), 16);
                //     secretBytes1.push(byte);
                // }
